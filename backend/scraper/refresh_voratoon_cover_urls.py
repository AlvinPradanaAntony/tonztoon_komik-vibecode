...
- Resume memakai checkpoint `backend/checkpoints/refresh_voratoon_cover_urls.json`.
...
Usage:
    ...
    python -m scraper.refresh_voratoon_cover_urls --verify-image --reset
    python -m scraper.refresh_voratoon_cover_urls --limit 100 --no-anti-blocking
"""
...
from app.services.image_service import (
    fetch_voratoon_cover_url_for_slug,
    get_proxy_headers,
    get_voratoon_cover_refresh_candidates,
...
CHECKPOINT_FILE = CHECKPOINT_DIR / "refresh_voratoon_cover_urls.json"
...
async def fetch_candidates(*, limit: int, after_id: int) -> list[CoverUrlCandidate]:
    async with async_session() as session:
        rows = await get_voratoon_cover_refresh_candidates(
            session,
            limit=limit,
            after_id=after_id,
        )
...
async def process_candidate(
    ...
    *,
    ...
    async with async_session() as session:
        changed = await update_voratoon_cover_url_for_slug(
            session,
            slug=candidate.slug,
            cover_url=fresh_url,
        )
...
async def run(args: argparse.Namespace) -> None:
    ...
    update_progress(
        ...
        state="starting",
        note="Refresh Voratoon cover URLs dimulai",
    )
    persist_checkpoint(checkpoint, enabled=checkpoint_enabled)

    logger.info("═" * 60)
    logger.info("🚀 Refresh Voratoon Cover URLs dimulai — %s", started_at.isoformat())
    ...
    finished_at = now_wib()
    logger.info("═" * 60)
    logger.info("🏁 Refresh Voratoon Cover URLs selesai!")
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Refresh comics.cover_image_url untuk signed cover URL Voratoon.",
    )
